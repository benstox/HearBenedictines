...
def getmp3name(x, urllist):
    lastpart = "/[^/]*\.mp3$"
    match = search(lastpart, urllist[x])
...
def getlecturename(mp3name):
    ...
    mp3name = sub('_', ' ', mp3name) # replaces underscores with spaces
    mp3name = sub('-', ' - ', mp3name) # replaces '-' with ' - '
    mp3name = sub(' 0+', ' ', mp3name) # replaces ' 0+' with ' ' to get rid of possible initial 0s
    mp3name = sub('([0-9]+)', 'Lecture \\1:', mp3name) # replaces numbers with 'Lecture ##:'
    mp3name = sub(' {2,}', ' ', mp3name) # gets rid of double (or more) spaces possibly caused by something above
    mp3name = sub('([a-z])([A-Z])', '\\1 \\2', mp3name) # adds a space between a lowercase followed by an uppercase
...
def searchfortitle(mp3name, linelist):
    for i in range(len(linelist)):
        if search(mp3name, linelist[i]):
            foundindex = i
    for i in range(foundindex, -1, -1):
        if search('<title>', linelist[i]):
            titleindex = i
            break
    return findall('^<title>(.*)</title>$', linelist[titleindex].strip())[0]
    
# converts the output of a getmp3name()
# into the format of 'Vespers for 8 December 2013' or suchlike
def getofficenamedate(mp3name, linelist):
    #print mp3name
    mp3name = mp3name[:-4]
    # this should sort out Le Barroux mp3s
    if search('Barroux', mp3name):
        sansbarroux = search('[0-9].*', mp3name)
        mp3name = sansbarroux.group()
        numbers = search('([0-9]*)-([0-9]*)-([0-9]*)', mp3name)
        year = numbers.group(1)
        month = numbers.group(2)
        day = int(numbers.group(3))
        officename = search('[A-Za-z].*', mp3name)
        try:
            name = OFFICES[officename.group()]
        except KeyError:
            #return "Office/Mass name not found in dictionary!"
            return searchfortitle(mp3name, linelist)
            #return mp3name
            
    # this should sort out Norcia mp3s
    else:
        numbers = search('[0-9]+', mp3name)
        if numbers:
        # it will go here if there are any numbers in the name
        # i.e. if there is a date in the name
            rawdate = numbers.group()
            correctedrawdate = sub('0+', '0', rawdate) # corrects for a typo seen in the past: Laudes201400422n.mp3, an extra zero, too many digits
            #print rawdate, correctedrawdate
            if len(rawdate) != 8 and len(correctedrawdate) != 8: 
                #return "ERROR!"
            ...
            if len(correctedrawdate) == 8:
                year = correctedrawdate[:4]
                month = correctedrawdate[4:6]
                day = int(correctedrawdate[6:])
            else:
                year = rawdate[:4]
                month = rawdate[4:6]
                day = int(rawdate[6:])
